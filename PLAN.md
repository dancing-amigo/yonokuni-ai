...
- [x] ルールベースBotや旧モデルとの対戦ジョブを追加しCI化（GitHub Actions `evaluate-rule-baseline` & `gate-model-baseline`）。
- [x] 勝率・平均手数・中央勝利率・死に駒発生ターンを TensorBoard/W&B に記録。
...
- [x] ログリプレイ機能（`scripts/play_vs_ai.py --log-file/--replay-log`）を実装。
...
## 5. 推論ユーティリティの強化
- [x] Mixed precision・バッチ推論・TorchScript/ONNX エクスポートに対応。
- [x] モデルのバージョニング用メタデータ（保存/読み込み）API を追加。
